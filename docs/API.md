...
| GET | `/v1/control-plane/services` | platform admin | Cursor-paged active panel owners with counts of active panels and registered gateway routes. `health=UNKNOWN`, `lastObservedAt=null` until a real metrics owner is connected. `limit` 1–100; `cursor` is a prior owner-service key; `page.nextCursor` is null on the last page |
