...
| GET | `/v1/sessions` | active session | Up to 100 of the caller's sessions with ID, UTC creation/expiry/revocation times, `current` and `active`; never returns cookie, token, CSRF value, IP or user-agent |
| DELETE | `/v1/sessions/:id` | owning session principal + CSRF + Origin | Revoke only a session owned by the caller. Repeating an already revoked session is safe. Revoking the current session clears its cookies |
...
| POST | `/v1/control-plane/services/:ownerService/operational-profiles` | platform admin + CSRF + Origin + idempotency key | Publish the next append-only operational profile version for an active registered service. Body carries `expectedCurrentVersion`, accountable team/on-call alias, HTTPS runbook, availability target in basis points, p95 latency target in milliseconds, and RTO/RPO in minutes. A stale expected version returns `409`; profile history is never overwritten |
...
| POST | `/v1/staff/:id/disable` | platform admin + CSRF + Origin | Emergency staff-only disable. Locks the principal, prevents self-disable, revokes every unrevoked session in the same transaction and emits audit/outbox evidence. Customer and agency lifecycle remains with its owning identity service |
| POST | `/v1/workload-identities` | platform admin + CSRF + Origin + idempotency key | Register one `WORKLOAD` principal for an active panel owner service. No credential or private key is created or returned; duplicate service registration returns 409 |
| POST | `/v1/workflow-runs/:id/steps/:stepKey/callbacks` | workload bearer token + idempotency key | Owning service reports `SUCCEEDED`, `FAILED` or `COMPENSATED` with an opaque `evidenceId` and UTC `occurredAt`. Core records immutable evidence and changes only orchestration state; a cross-owner callback is denied |
...
The service catalog returns `operationalReadiness=CONFIGURED` and the latest operational profile only after one has been explicitly published; otherwise it returns `UNCONFIGURED` and `operationalProfile=null`. The core does not invent a universal SLO or infer availability from a missing profile.
...
Workload callback authentication uses an externally issued Ed25519 JWT. The protected header must be `alg=EdDSA`, `typ=JWT` and carry a known `kid`. Claims are exact `iss` and `aud`, registered workload principal UUID in `sub`, matching owner-service slug in `service`, UUID `jti`, integer `iat`/`exp` and optional integer `nbf`. Lifetime is at most `WORKLOAD_TOKEN_MAX_TTL_SECONDS` (30–900 seconds). The core accepts public keys only from `WORKLOAD_JWKS_JSON`; private signing keys and service credentials never belong in this repository or database. A callback must be retried with the same idempotency key and identical body.
